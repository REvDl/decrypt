...
def test_auto_and_dry_run_warns_but_does_not_raise(monkeypatch, capsys):
    parser = build_parser()
...
def test_get_mode_various_flags(args, expected):
    parser = build_parser()
...
def test_slang_mode_does_not_call_launcher_functions(mock_execute, mock_commit):
    handle_result(
...
@patch("decrypt.launcher.process_commit")
@patch("decrypt.launcher.execute_command_prompt")
def test_commit_mode_calls_process_commit(mock_execute, mock_commit):
    handle_result(
